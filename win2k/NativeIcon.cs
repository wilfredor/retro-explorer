...
namespace ex_plorer
{
    internal static class NativeIcon
    {
        ...
        private static Icon GetIcon(string path, SHGFI flags)
        {
            ...
            SHGetFileInfo(path, 0u, ref sfi, (uint)Marshal.SizeOf(typeof(SHFILEINFO)), flags | SHGFI.USEFILEATTRIBUTES);
        ...
        {
            SHFILEINFO sfi = new SHFILEINFO();
            SHGetFileInfo(path, 128u, ref sfi, (uint)Marshal.SizeOf(typeof(SHFILEINFO)), SHGFI.ICON | SHGFI.SMALLICON | SHGFI.USEFILEATTRIBUTES);
            smallIcon = CloneAndDestroyIcon(sfi.hIcon);
            SHGetFileInfo(path, 128u, ref sfi, (uint)Marshal.SizeOf(typeof(SHFILEINFO)), SHGFI.ICON | SHGFI.TYPENAME | SHGFI.USEFILEATTRIBUTES);
